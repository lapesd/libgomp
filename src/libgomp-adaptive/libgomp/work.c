...
#ifdef PROFILE
  ws->runtime_overheads = gomp_malloc (nthreads * sizeof (uint64_t));
  bzero (ws->runtime_overheads, nthreads * sizeof (uint64_t));
#endif /* PROFILE */
...
free_work_share (struct gomp_team *team, struct gomp_work_share *ws)
{
  ...
  {
#if defined(LIBGOMP_USE_ADAPTIVE)
    if (ws->adaptive_chunks !=0) free(ws->adaptive_chunks);
...
gomp_work_share_end (void)
{
  ...
  if (gomp_barrier_last_thread (bstate))
    {
#ifdef PROFILE
      total_loop_time = gomp_get_tick () - thr->ts.work_share->total_loop_time;
      int i;
      fprintf (stderr, "%% Loop %%\n");
      for (i = 0; i < team->nthreads; i++) {
	fprintf (stderr, "[%i] %lu\n", i, thr->ts.work_share->runtime_overheads[i]);
	//	fprintf (stderr, "[%i] %f\n", i, (float) thr->ts.work_share->runtime_overheads[i] * 100.0 / (float) total_loop_time);
      }
      fprintf (stderr, "%%%%%%%%%%\n\n");
#endif /* PROFILE */
