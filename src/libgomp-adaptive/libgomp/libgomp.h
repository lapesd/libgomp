...
/** Some macro for extension
*/
// to add timings in loop...
//#define LIBGOMP_PROFILE_LOOP 1
#define LIBGOMP_FORCE_ALIGNED 1
#define LIBGOMP_USE_ADAPTIVE 1 /* to activate adaptive loop schedule */
#if defined(LIBGOMP_USE_ADAPTIVE)
#  define LIBGOMP_USE_NUMA 1 /* to activate numa aware + pws */
//#  define LIBGOMP_USE_PWS_STRICT 1 /* do not steal outside numa node */
#endif
 
//#define assert_value( var, value) if (var >= value) abort()
#define assert_value( var, value) 
...
extern struct gomp_profile_data profile_loop[64][256]; /* assume less than 256 core */
extern int work_shared_index;  /* next work_share index */

#define PF(thr) (&profile_loop[thr->ts.work_share->ws_index % 64][thr->ts.team_id])
...
struct gomp_work_share
{
  ...
  union {
    ...
    struct {
      ...
      unsigned long long start_t0_ull;
...
#if defined(LIBGOMP_PROFILE_LOOP)
  int ws_index; /* incremented each time a new workshared is allocated to a team */
#endif
};
...
extern unsigned short gomp_init_thread_affinity (pthread_attr_t *);
...
extern void libgomp_iter_next (void);
